...
namespace OrderingSystemProject.Services
{
	public class KitchenService : IKitchenServices
	{
		...
		public void TakeCat(int _order_id, int _cat)
		{
			...
			for (int i = 0; i < order.Items.Count; i++) // Go throu list of order_item of the order
			{
				if (order.Items[i].MenuItem.Card == ItemCard.DRINKS || order.Items[i].MenuItem.Card == ItemCard.ALCOHOLIC_DRINKS || (int)order.Items[i].MenuItem.Category != _cat) continue; // if item is drink or not cat, skip it
